SELECT
-- ERP_ORDER,ARTICULO,DESCRIPCION,CORTO,POR_SURTIR,SURTIENDOSE,POR_CARGAR,POR_ENVIAR,ENVIADO,TOTAL, INV_TUL,FECHA_ENVIO,
-- ERP_ORDER ARTICULO DESCRIPCION CORTO POR_SURTIR SURTIENDOSE POR_CARGAR POR_ENVIAR ENVIADO TOTAL INV_TUL FECHA_ENVIO
    ERP_ORDER,
    ARTICULO,
    DESCRIPCION,
...
(
 ...
 LEFT OUTER JOIN (
  SELECT
  LI.ITEM, CAST(SUM(LI.on_hand_qty) AS INT) AS INV_TUL
  ...
  GROUP BY ITEM
) AS INV_TULTI ON INV_TULTI.ITEM = CP.ARTICULO
