SELECT 
  ...
  CONCAT('''', SD.internal_shipment_line_num, '''')  AS  Detail_internal_shipment_line_num1, SH.trailing_sts AS Header_trailing_sts, SH.leading_sts  AS Header_leading_sts, SH.internal_shipment_num As  Header_internal_shipment_num, SH.shipment_id  AS Header_shipment_id,  SH.shipping_load_num  AS Header_shipping_load_num,SD.ITEM AS  Detail_item,SD.status1 AS  Detail_status1,SD.internal_shipment_line_num  AS  Detail_internal_shipment_line_num,SC.status AS Container_status,  SC.item  AS Container_item,SD.total_qty AS  Detail_QTY,SC.location AS Container_location,SC.internal_container_num   AS Container_internal_container_num,SC.parent_container_id  AS Container_parent_container_id
  -- SC.item, CAST(SD.total_qty AS INT), SC.location
